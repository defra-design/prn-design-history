...
/**
 * Determines whether the given node is considered to be a for loop "afterthrough" by the logic of this rule.
 ...
 * @param {ASTNode} node The node to check.
 * @returns {boolean} `true` if the node is a for loop afterthrough.
 */
function isForLoopAfterthrough(node) {
    const parent = node.parent;

    if (parent.type === "SequenceExpression") {
        return isForLoopAfterthrough(parent);
    }
...
module.exports = {
    ...
    create(context) {
        ...
        let allowForLoopAfterthroughs = false;

        if (typeof config === "object") {
            allowForLoopAfterthroughs = config.allowForLoopAfterthroughs === true;
        }

        return {

            UpdateExpression(node) {
                if (allowForLoopAfterthroughs && isForLoopAfterthrough(node)) {
                    return;
                }
